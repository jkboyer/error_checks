...
db <- odbcConnectAccess("./data/GC20210403.mdb", case = "nochange")
...
turbidity <- read_csv("./data/GC20210403_turbidity_temperature.csv")
...
scan <- read_csv("./data/GC20210403_scanner_downloads.csv")
...
turbidity$START_DATE <- as.Date(turbidity$START_DATE)
...
#do not match - did we download the HPRlite used for angling?
#that could account for gap in sample size
...
tag.errors <- tag.errors %>%
  dplyr::select(ACCESS_FISH_ID, PITTAG, PITTAG_RECAP, SPECIES_CODE, TOTAL_LENGTH, DATASHEET_SAMPLE_ID,
                START_DATETIME, START_DATE, GEAR_CODE, CREW) %>%
  #angling fish without matches are not errors, we did not download the HPRlite
  #scanner used for angling
  filter(GEAR_CODE != "AN")
#remaining two tags are probably correct - from 4/5/21 boat B, when the boat
#swamped and the scanner got killed by water so we were not able to download this
#scanner
...
duplicate.tags <- duplicate.tags %>%
  dplyr::select(ACCESS_SAMPLE_ID, ACCESS_FISH_ID,PITTAG, PITTAG_RECAP, SPECIES_CODE, TOTAL_LENGTH, DATASHEET_SAMPLE_ID,
         START_DATETIME, START_DATE, GEAR_CODE, CREW)
#3DD.003D7D349F is fine - same fish recaptured on another day
#3DD.003D7D4418 - RBT from 4/3 is error - need to check paper data
#    no clues in scanner download - maybe real tag was in the scanner that
#    got swamped/flooded two days later
